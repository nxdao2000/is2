...
setMethod(
          ...
          definition=function (x, ...) {
            ...
            if (length(y)==0) {
              ...
              p <- sapply(y,is,'is3')
              pl <- sapply(y,is,'is3List')
              if (!all(p||pl))
                stop("cannot mix ",sQuote("is3"),
                     " and non-",sQuote("is3")," objects")
              y[p] <- lapply(y[p],list)
              y[pl] <- lapply(y[pl],as,"list")
              new("is3List",c(list(x),y,recursive=TRUE))
            }
          }
          )
...
setMethod(
          ...
          definition=function (x, ...) {
            ...
            if (length(y)==0) {
              ...
              y[p] <- lapply(y[p],list)
              y[pl] <- lapply(y[pl],as,"list")
              new("is3List",c(as(x,"list"),y,recursive=TRUE))
            }
          }
          )
...
setMethod(
          ...
          definition=function(x, i, ...) {
            new('is3List',as(x,"list")[i])
          }
          )

setMethod(
          'conv.rec',
          signature=signature(object='is3List'),
...
predvarplot.is3 <- function (object, pars, type = 'l', mean = FALSE, ...) {
  if (!is(object,'is3'))
    stop("predvarplot error: ",sQuote("object")," must be of class ",sQuote("is3"),call.=FALSE)
  if (missing(pars))
    pars <- object@pars
...
is3.diagnostics <- function (z) {
  ## assumes that z is a list of is3s with identical structure
  ...
  while (hi<nplots) {
    ...
    for (i in seq(from=low,to=hi,by=1)) {
      ...
      do.xax <- (n%%nr==0||n==n.per.page)
      if (do.xax) axis(1,xpd=NA)
      if (do.xax) mtext("is3 iteration",side=1,line=3)
    }  
    low <- hi+1
    mtext("is3 convergence diagnostics",3,line=2,outer=TRUE)
  }
  invisible(NULL)
}
