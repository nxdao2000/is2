# aif algorithm functions

# define the aif class
setClass(
    'aif',
...
aif.cooling <- function (frac, nt, m, n){   # cooling schedule for aif
...
aif.internal <- function (
    ...
    if (!missing(cooling.factor)) {
        warning(sQuote("cooling.factor")," is deprecated.\n",
            "See ",sQuote("?aif")," for instructions on specifying the cooling schedule.",
            call.=FALSE)
    ...
    cooling <- cooling.function(
        type=cooling.type,
        perobs=(method=="aif")||(method=="ris1")||(method=="is3"),
    ...
    if (!all(is.finite(theta[c(pars,ivps)]))){
        stop(
            sQuote("aif")," cannot estimate non-finite parameters.\n",
    ...
    if (Nis>0) {
        tmp.aif <- new("aif",object,particles=particles,Np=Np[1L])
    } 
    else{
        pfp <- obj
    }
    ...
    for (n in seq_len(Nis)){ ## iterate the smoothing
        ...
        if (verbose) cat("aif iteration ",n," of ",Nis," completed\n")
...
setMethod(
    "aif",
    signature=signature(object="aif"),
    function (object, Nis,
              start,
              pars, ivps,
              particles, rw.sd,
...
aif.profileDesign <- function (object, profile, lower, upper, nprof, ivps, 
