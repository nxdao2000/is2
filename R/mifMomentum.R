# mifMomentum algorithm functions

# define the mifMomentum class
setClass(
    'mifMomentum',
...
mifMomentum.cooling <- function (frac, nt, m, n){   # cooling schedule for mifMomentum
...
mifMomentum.internal <- function (
    ...
    if (!missing(cooling.factor)) {
        warning(sQuote("cooling.factor")," is deprecated.\n",
            "See ",sQuote("?mifMomentum")," for instructions on specifying the cooling schedule.",
            call.=FALSE)
    ...
    cooling <- cooling.function(
        type=cooling.type,
        perobs=(method=="mifMomentum")||(method=="ris1")||(method=="is3"),
    ...
    if (!all(is.finite(theta[c(pars,ivps)]))){
        stop(
            sQuote("mifMomentum")," cannot estimate non-finite parameters.\n",
    ...
    if (Nis>0) {
        tmp.mifMomentum <- new("mifMomentum",object,particles=particles,Np=Np[1L])
    } 
    else{
        pfp <- obj
    }
    ...
    for (n in seq_len(Nis)){ ## iterate the smoothing
        ...
        paramMatrix <- pfp@paramMatrix
               ...
               if (lag>=0){
                 ...
                 gradient[c(pars,ivps)] <- -(phat[c(pars,ivps)]-ntimes*oldtheta[c(pars,ivps)])/ntimes  
                 
                 oldtheta1[c(pars,ivps)] <- 0.05*oldtheta1[c(pars,ivps)]+gradient[c(pars,ivps)]
                 
                 theta[c(pars,ivps)]  <-  theta[c(pars,ivps)]-oldtheta1[c(pars,ivps)]
               }
        ...
        if (verbose) cat("mifMomentum iteration ",n," of ",Nis," completed\n")
...
setMethod(
    ...
    function (object, Nis = 1,
            ...
            method = c("mifMomentum","ris1","is1", "is3"),
...
setMethod(
    "mifMomentum",
    signature=signature(object="mifMomentum"),
    function (object, Nis,
              start,
              pars, ivps,
              particles, rw.sd,
...
mifMomentum.profileDesign <- function (object, profile, lower, upper, nprof, ivps, 
