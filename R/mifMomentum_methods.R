## this file contains short definitions of methods for the 'mifMomentum' class
...
setMethod('conv.rec','mifMomentum',
          function (object, pars, transform = FALSE, ...) {
              conv.rec.internal(object=object,pars=pars,transform=transform,...)
          }
          )

## plot mifMomentum object
setMethod(
    "plot",
    "mifMomentum",
    function (x, y, ...) {
        if (!missing(y))
            warning("in ",sQuote("plot-mifMomentum"),": ",
                    sQuote("y")," is ignored",call.=FALSE)
        mifMomentum.diagnostics(list(x))
    }
)

## mifMomentumList class
setClass(
    'mifMomentumList',
    contains='list',
    validity=function (object) {
        if (!all(sapply(object,is,'mifMomentum'))) {
            retval <- paste0(
                "error in ",sQuote("c"),
        ...
        if (!all(apply(d,1,diff)==0)) {
            retval <- paste0(
                "error in ",sQuote("c"),
                ": to be combined, ",sQuote("mifMomentum"),
...
setMethod(
    'c',
    signature=signature(x='mifMomentum'),
    definition=function (x, ...) {
        y <- list(...)
        if (length(y)==0) {
            new("mifMomentumList",list(x))
        } else {
            p <- sapply(y,is,'mifMomentum')
            pl <- sapply(y,is,'mifMomentumList')
            if (!all(p||pl))
                stop("in ",sQuote("c"),": cannot mix ",sQuote("mifMomentum"),
                     " and non-",sQuote("mifMomentum")," objects",call.=FALSE)
            y[p] <- lapply(y[p],list)
            y[pl] <- lapply(y[pl],as,"list")
            new("mifMomentumList",c(list(x),y,recursive=TRUE))
        }
    }
)
...
setMethod(
    'c',
    signature=signature(x='mifMomentumList'),
    definition=function (x, ...) {
        y <- list(...)
        if (length(y)==0) {
            x
            ...
            y[p] <- lapply(y[p],list)
            y[pl] <- lapply(y[pl],as,"list")
            new("mifMomentumList",c(as(x,"list"),y,recursive=TRUE))
        }
    }
)
...
setMethod(
    ...
    definition=function(x, i, ...) {
        new('mifMomentumList',as(x,"list")[i])
    }
)

setMethod(
    'conv.rec',
    signature=signature(object='mifMomentumList'),
...
setMethod(
    ...
    definition=function (x, y, ...) {
        if (!missing(y))
            warning("in ",sQuote("plot-mifMomentum"),": ",
                    sQuote("y")," is ignored",call.=FALSE)
        mifMomentum.diagnostics(x)
    }
)

## predvarplot.mifMomentum <- function (object, pars, type = 'l', mean = FALSE, ...) {
##   if (!is(object,'mifMomentum'))
##     stop("predvarplot error: ",sQuote("object")," must be of class ",sQuote("mifMomentum"),call.=FALSE)
##   if (missing(pars))
##     pars <- object@pars
...
mifMomentum.diagnostics <- function (z) {
    ## assumes that z is a list of mifMomentums with identical structure
    ...
    ## plot mifMomentum convergence diagnostics
    ...
    while (hi<nplots) {
        ...
        for (i in seq(from=low,to=hi,by=1)) {
            ...
            do.xax <- (n%%nr==0||n==n.per.page)
            if (do.xax) axis(1,xpd=NA)
            if (do.xax) mtext("mifMomentum iteration",side=1,line=3)
        }  
        low <- hi+1
        mtext("mifMomentum convergence diagnostics",3,line=2,outer=TRUE)
    }
    invisible(NULL)
}
