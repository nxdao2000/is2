## this file contains short definitions of methods for the 'avif' class
...
setMethod(
    ...
    function (x, y, ...) {
        if (!missing(y))
            warning("in ",sQuote("plot-avif"),": ",
                    sQuote("y")," is ignored",call.=FALSE)
        avif.diagnostics(list(x))
    }
)
...
setMethod(
    ...
    definition=function (x, ...) {
        ...
        if (length(y)==0) {
            ...
            p <- sapply(y,is,'avif')
            pl <- sapply(y,is,'avifList')
            if (!all(p||pl))
                stop("in ",sQuote("c"),": cannot mix ",sQuote("avif"),
                     " and non-",sQuote("avif")," objects",call.=FALSE)
            y[p] <- lapply(y[p],list)
            y[pl] <- lapply(y[pl],as,"list")
            new("avifList",c(list(x),y,recursive=TRUE))
        }
    }
)
...
setMethod(
    'c',
    signature=signature(x='avifList'),
    definition=function (x, ...) {
        y <- list(...)
        if (length(y)==0) {
            x
            ...
            y[p] <- lapply(y[p],list)
            y[pl] <- lapply(y[pl],as,"list")
            new("avifList",c(as(x,"list"),y,recursive=TRUE))
        }
    }
)
...
setMethod(
    ...
    definition=function(x, i, ...) {
        new('avifList',as(x,"list")[i])
    }
)

setMethod(
    'conv.rec',
    signature=signature(object='avifList'),
...
## predvarplot.avif <- function (object, pars, type = 'l', mean = FALSE, ...) {
##   if (!is(object,'avif'))
##     stop("predvarplot error: ",sQuote("object")," must be of class ",sQuote("avif"),call.=FALSE)
##   if (missing(pars))
##     pars <- object@pars
...
avif.diagnostics <- function (z) {
    ## assumes that z is a list of avifs with identical structure
    ...
    ## plot avif convergence diagnostics
    ...
    while (hi<nplots) {
        ...
        for (i in seq(from=low,to=hi,by=1)) {
            ...
            do.xax <- (n%%nr==0||n==n.per.page)
            if (do.xax) axis(1,xpd=NA)
            if (do.xax) mtext("avif iteration",side=1,line=3)
        }  
        low <- hi+1
        mtext("avif convergence diagnostics",3,line=2,outer=TRUE)
    }
    invisible(NULL)
}
