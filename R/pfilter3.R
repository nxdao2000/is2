...
pfilter3.internal <- function (object, params, Np,
                              tol, max.fail,
                              pred.mean = FALSE,
                              pred.var = FALSE,
                              filter.mean = FALSE,
  ...
  if (!is.null(seed))
    warning("in ",sQuote("pfilter3"),": argument ",sQuote("seed"),
  ...
  if (length(params)==0)
    stop(sQuote("pfilter3")," error: ",sQuote("params")," must be specified",call.=FALSE)

  if (missing(tol))
    stop(sQuote("pfilter3")," error: ",sQuote("tol")," must be specified",call.=FALSE)
  ...
  if (is.null(paramnames))
    stop(sQuote("pfilter3")," error: ",sQuote("params")," must have rownames",call.=FALSE)
  ...
  if (random.walk) {
    ...
    if (is.null(rw.names)||!is.numeric(.rw.sd))
      stop(sQuote("pfilter3")," error: ",sQuote(".rw.sd")," must be a named vector",call.=FALSE)
    if (!all(rw.names%in%paramnames))
      stop(
           sQuote("pfilter3")," error: the rownames of ",
  ...
  for (nt in seq_len(ntimes)) { ## main loop
    ...
    if (inherits(X,'try-error'))
      stop(sQuote("pfilter3")," error: process simulation error",call.=FALSE)
    gnsi.rproc <- FALSE
    ...
    if (pred.var) { ## check for nonfinite state variables and parameters
      ...
      if (length(problem.indices)>0) {  # state variables
        stop(
             sQuote("pfilter3")," error: non-finite state variable(s): ",
      ...
      if (random.walk) { # parameters (need to be checked only if 'random.walk=TRUE')
        ...
        if (length(problem.indices)>0) {
          stop(
               sQuote("pfilter3")," error: non-finite parameter(s): ",
    ...
    if (inherits(weights,'try-error'))
      stop("in ",sQuote("pfilter3"),": error in calculation of weights.",call.=FALSE)
    if (!all(is.finite(weights)))
      stop("in ",sQuote("pfilter3"),": ",sQuote("dmeasure")," returns non-finite value.",call.=FALSE)
    gnsi.dmeas <- FALSE
    ...
    if (inherits(xx,'try-error')) {
      stop(sQuote("pfilter3")," error",call.=FALSE)
    }
    all.fail <- xx$fail
    loglik[nt] <- xx$loglik
    ...
    if (all.fail) { ## all particles are lost
      ...
      if (nfail>max.fail)
        stop(sQuote("pfilter3")," error: too many filtering failures",call.=FALSE)
    }
    ...
    if (verbose && (nt%%5==0))
      cat("pfilter3 timestep",nt,"of",ntimes,"finished\n")
...
setMethod(
          "pfilter3",
          signature=signature(object="pfilter3d.pomp"),
          function (object, params, Np, tol, ...) {
            if (missing(params)) params <- coef(object)
            if (missing(Np)) Np <- object@Np
            if (missing(tol)) tol <- object@tol
            f <- selectMethod("pfilter3","pomp")
            f(
              object=object,
              params=params,
              Np=Np,
